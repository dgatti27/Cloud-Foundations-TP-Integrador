...
locals {
  # Policies JSON del lab viven en ecs/ (padre de iac/)
  ecs_dir = "${path.module}/.."

  # Paths stand-in ≈ access points /airflow/dags y /airflow/logs
  standin_dags = "${local.ecs_dir}/efs-standin/dags"
  standin_logs = "${local.ecs_dir}/efs-standin/logs"
}
...
resource "local_file" "efs_standin_marker" {
  filename = "${local.ecs_dir}/efs-standin/.iac-managed"
  content  = <<-EOT
    Managed by OpenTofu ecs/iac lab-09b-tp (enable_ecs_api=${var.enable_ecs_api}).
    ...
    Demos → python ecs/ecs_demo.py --skip-infra
  EOT
}
...
resource "local_file" "efs_inventory" {
  ...
  content = jsonencode({
    ...
    standin_logs         = "ecs/efs-standin/logs"
