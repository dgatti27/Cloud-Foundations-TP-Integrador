...
- **Servidor 1 (datos)**: base **CRUDA** en PostgreSQL + **Datawarehouse** en PostgreSQL.
...
- Flujo: origenes -> **ETL grupo 1** -> base CRUDA -> **ETL grupo 2** (procesa) -> DW.
...
|---|---|---|
| Dos servidores sin replica | La caida del server de datos tira base cruda + DW | Separar computo (EC2) de datos (RDS Multi-AZ) |
...
| Credenciales en el server | Fuga de secretos | Secrets Manager + IAM |
...
```
ERP(FoxPro) / Ecommerce(Mongo) / Eventos(Mongo) / Scraping  (origenes on-host)
        │  conexion por host (VPN / Direct Connect, via NAT GW)
        ▼
Airflow en ECS Fargate (DAGs Python)  ──stage──►  S3
        │  ETL g1: ingesta cruda        ▲
        ▼                               │ ETL g2: procesa
RDS PostgreSQL  [ base CRUDA ] ────────►[ Datawarehouse ]  (Multi-AZ) ──snapshots──► S3
                                              ▲
                                              │  query
                              Lambda API (dw-api) ◄─HTTPS─ ALB ◄─por host─ Qlik BI
```
