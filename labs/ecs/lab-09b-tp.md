...
> |---|---|---|
> | ECS Fargate (Airflow) | `compose.yaml` servicios `airflow-*` | Mismo proceso; sin API ECS |
...
| Archivo | Rol | Por qué |
|---|---|---|
| [`../compose.yaml`](../compose.yaml) (`airflow-*`) | webserver + scheduler | Sustituto Hobby de ECS service |
...
```powershell
# Desde la raíz del repo (stack único)
docker compose up -d
...
> Los volúmenes EFS (`./ecs/efs-standin/dags` y `logs`) están en `compose.yaml`.
...
```powershell
# Solo Airflow (no bajes LocalStack / MinIO / RDS)
...
| `compose.yaml` (`airflow-*`) | ≈ Fargate (+ mount `etl/`) |
