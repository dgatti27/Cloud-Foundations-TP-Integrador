...
Orquesta en LocalStack Hobby el equivalente local de:
  ...
  --------------------------------   -----------------------------------------
  ECS Fargate (Airflow)              compose.yaml (servicios airflow-*)
...
EFS_DAGS = ROOT / "apps" / "airflow" / "dags"
EFS_LOGS = ROOT / "apps" / "airflow" / "logs"
...
def step_airflow_up() -> None:
    ...
    Detalles (servicios airflow-* en compose.yaml de la raíz):
    ...
    _run(_compose_cmd("up", "-d", *AIRFLOW_SERVICES), capture=False)
...
def step_cleanup() -> None:
    """Apaga solo Airflow. No hace down del stack (LocalStack/MinIO/RDS quedan)."""
    print("\n6. Cleanup — stop Airflow (resto del compose intacto)")
    _run(_compose_cmd("stop", "airflow-webserver", "airflow-scheduler"), capture=False)
    print("  ✓ Airflow detenido. Emuladores / VPC / RDS intactos.")
