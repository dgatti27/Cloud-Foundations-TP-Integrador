# =============================================================================
# main.tf — VPC Multi-AZ lab-07-v2 (pasos 1–7 del lab)
# -----------------------------------------------------------------------------
# Mapa CIDR:
#   VPC 10.0.0.0/16
...
locals {
  # AZs derivadas de la región (lab: AZ_A / AZ_B)
  az_a = "${var.region}a"
  az_b = "${var.region}b"
}
...
# Tag Name=tp-integrador-vpc → lookup de labs (describe-vpcs filter tag:Name).
# ---------------------------------------------------------------------------
...
# Tags Role = lookup de labs siguientes (rds, ecs-lambda-efs, alb).
# ---------------------------------------------------------------------------
...
# Attach implícito vía vpc_id. Base de entrada ALB y del NAT.
# ---------------------------------------------------------------------------
...
# Subred = pública IFF 0.0.0.0/0 → IGW. Asociar a public-alb-a/b.
# ---------------------------------------------------------------------------
...
# RT_RDS: solo local (+ vpce S3 luego). Sin Internet.
...
# enable_nat=false → count=0 (ahorra modelo / evita flaky LocalStack).
# ---------------------------------------------------------------------------
...
resource "aws_vpc_security_group_ingress_rule" "api_from_alb" {
  ...
  # LocalStack a veces reescribe el ref; evita drift eterno en plan
  lifecycle {
...
# ---------------------------------------------------------------------------
# Inventario para labs siguientes (RDS, Lambda, ECS)
# Escribe vpc/vpc_config.json (mismo schema que usa rds_tp_demo / lambda_demo).
# ---------------------------------------------------------------------------
resource "local_file" "vpc_config" {
  count = var.write_vpc_config ? 1 : 0

  filename = "${path.module}/../vpc_config.json"
  content = jsonencode({
    vpc_id      = aws_vpc.main.id
    region      = var.region
    nat         = try(aws_nat_gateway.main[0].id, null)
    endpoint_s3 = aws_vpc_endpoint.s3.id
    subnets = {
    ...
    notes = "Generado por OpenTofu vpc/iac (lab-07-v2). Lake S3 = MinIO, no este vpce."
  })
}
