...
resource "null_resource" "rds_seed" {
  ...
  triggers = {
    ...
    seed_checksum = filesha256("${local.repo_root_abs}/rds/seed_tp.sql")
  }
  ...
  provisioner "local-exec" {
    ...
    environment = {
      ...
      POST_RDS_SEED            = "${replace(local.repo_root_abs, "\\", "/")}/rds/seed_tp.sql"
...
module "lambda_api" {
  ...
  lambda_source_dir = "${local.repo_root_abs}/lambda"
...
# Inventario consumible por demos (mismo shape que vpc/vpc_config.json)
resource "local_file" "vpc_config" {
  ...
  content = jsonencode({
    ...
    notes       = "Generado por OpenTofu lab-09-tp. EFS/ECS API = enable_ecs_api; Hobby usa stand-in."
  })
}

resource "local_file" "vpc_config_repo" {
  filename = "${local.repo_root_abs}/vpc/vpc_config.json"
