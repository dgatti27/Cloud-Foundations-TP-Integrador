...
case "$cmd" in
  apply)
    wait_backends
    exec python iac/iac_demo.py "$@"
    ;;
  apply-reconcile)
    wait_backends
    exec python iac/iac_demo.py --reconcile "$@"
    ;;
  plan)
    wait_backends
    exec python iac/iac_demo.py --plan "$@"
    ;;
  destroy)
    wait_backends
    exec python iac/iac_demo.py --destroy "$@"
    ;;
  init)
    wait_backends
    cd iac/tp && exec tofu init -upgrade
    ;;
  tofu)
    wait_backends
    cd iac/tp && exec tofu "$@"
  ...
  help|-h|--help)
    cat <<'EOF'
Uso: docker compose -f compose.yaml -f docker-compose.iac.yaml run --rm tp-iac <comando>

  apply             tofu init/apply vía iac_demo.py (default)
  apply-reconcile   limpia choques de demos imperativos + apply
  plan              solo plan
  destroy           destroy -auto-approve
  init              solo tofu init en iac/tp
  tofu <args…>      pasa args a tofu (cwd=iac/tp)
