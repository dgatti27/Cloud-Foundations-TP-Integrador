...
locals {
  ...
  lambda_dir_posix  = replace(abspath(local.lambda_dir), "\\", "/")
}
...
resource "aws_iam_role" "api" {
  ...
  assume_role_policy = file("${local.lambda_dir}/trust_lambda.json")
...
resource "aws_iam_role_policy" "api_execution" {
  ...
  policy = file("${local.lambda_dir}/execution_policy.json")
}
...
resource "aws_iam_role_policy" "api_secrets" {
  ...
  role   = aws_iam_role.api.id
  policy = file("${local.lambda_dir}/task_api_policy.json")
}
...
resource "aws_iam_group_policy" "bi_api_invoke" {
  ...
  policy = file("${local.lambda_dir}/group_bi_api_policy.json")
}
...
data "archive_file" "lambda_zip_lite" {
  ...
  source {
    content  = file("${local.lambda_dir}/query_gold.py")
...
resource "null_resource" "lambda_zip_pg8000" {
  ...
  triggers = {
    handler = filesha256("${local.lambda_dir}/handler.py")
    query   = filesha256("${local.lambda_dir}/query_gold.py")
  ...
  provisioner "local-exec" {
    ...
    environment = {
      LAMBDA_SRC_DIR     = local.lambda_dir_posix
...
resource "local_file" "lambda_inventory" {
  filename = "${local.lambda_dir}/lambda_inventory.json"
  content = jsonencode({
    lab            = "api-tp"
    ...
    alb_standin    = "python lambda/lambda_demo.py --skip-infra  # Compose :8088"
